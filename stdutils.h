...
uint8_t decimal_to_bcd(uint8_t dcml)				//returns bcd of decimal number
{
	uint8_t lsnib = dcml % 10;                      // decimal value of each nibble
	uint8_t msnib = dcml / 10;
	uint8_t bcd = (msnib << 4) + lsnib;				// recombine as BCD
	return(bcd);
}
...
// {
// 	DDRB |= (1<<DDB3)|(1<<DDB5)|(1<<DDB2);	/* Make MOSI, SCK, SS as Output pin */
// 	DDRB |= (1<<DDB4);						/* Make MISO pin as input pin */
// 	PORTB |= (1<<PORTB2);					/* Make high on SS pin */
// 	SPCR |= (1<<SPE)|(1<<MSTR)|(1<<SPR0)|(1<<SPR1);	/* Enable SPI in master mode with Fosc/16 */
